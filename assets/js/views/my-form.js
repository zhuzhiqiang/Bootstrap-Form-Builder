define([
    ...
    , "text!templates/app/renderformForShow.html"
], function ($, _, Backbone, banner
    , TempSnippetView
    , PubSub
    , _renderForm, _renderFormForShow) {
    return Backbone.View.extend({
        ...
        , initialize: function () {
            ...
            this.renderFormForShow = _.template(_renderFormForShow);
            this.render();
        }
        ...
        , render: function () {
            ...
            //     }).join("\n")
            // }));
            $("#renderCode").val(that.renderForm({
                multipart: this.collection.containsFileType(),
                text: _.map(this.collection.renderAllClean(), function (e) {
                    return e.html()
                }).join("\n")
            }));
            // showEffect.document.getElementById("render").innerHTML = that.renderFormForShow({
            //     multipart: this.collection.containsFileType(),
            //     text: _.map(this.collection.renderAllClean(), function (e) {
            //         return e.html()
            //     }).join("\n")
            ...
            this.$el.appendTo("#build .comps");
            // showEffect.window.setUpSwiper();
            // banner.setUpSwiper();
            this.delegateEvents();
        }
